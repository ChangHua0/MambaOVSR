...
class BasicLayer(nn.Module):
    def __init__(self,
        ...
        for i in range(depth):
            self.blocks.append(VSSBlock( # 如果SS2D每层传入的方向不同需要再这里传入
                hidden_dim=dim,
...
class RSSG_Blocks(nn.Module):
    def __init__(self, num_blocks, args):
        super(RSSG_Blocks, self).__init__()
        self.args = args
        self.layers = nn.ModuleList()
        self.nf = self.args.nf
        self.embed_dim = self.args.nf
        self.num_blocks = num_blocks
        self.patch_norm = True

        self.conv_first = nn.Conv2d(self.nf, self.embed_dim, 3, 1, 1) # nf->emb_dim
        self.conv_last = nn.Conv2d(self.embed_dim, self.nf, 3, 1, 1) # emb_dim->nf
        self.patch_embed = PatchEmbed(
            img_size=64,
            patch_size=1,
