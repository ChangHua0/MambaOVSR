...
#----------------------------------LocalScan-----------------------------------
from multi_mamba import MultiScan
import warnings
...
class SS2D_LocalScan(nn.Module):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
        d_state=16,
        ssm_ratio=2.0, # expand
        dt_rank="auto",
        act_layer=nn.SiLU,
        # dwconv ===============
        ...
        **kwargs,
        ...
        if directions==None: # 默认的扫描方向
            directions = ('h', 'h_flip', 'w2', 'w2_flip')
        ...
        # Local Mamba
        self.multi_scan = MultiScanVSSM(d_expand, choices=directions) # 传参进来四个方向
...
class VSSBlock_LocalScan(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
            norm_layer: Callable[..., torch.nn.Module] = partial(nn.LayerNorm, eps=1e-6),
            attn_drop_rate: float = 0, # ssm_drop_rate=0.0, 
            d_state: int = 16, # ssm_d_state=16,
            mlp_ratio: float = 2., # ssm_ratio=2.0,
        ...
        self.ln_1 = norm_layer(hidden_dim)
        # ----------------------------区别在这里-----------------------
        self.self_attention = SS2D_LocalScan(d_model=hidden_dim, d_state=d_state,ssm_ratio=mlp_ratio,dt_rank=ssm_dt_rank,act_layer=ssm_act_layer,
                                             d_conv=ssm_conv,conv_bias=ssm_conv_bias,
                                             dropout=attn_drop_rate,
                                                simple_init=ssm_simple_init, **kwargs)
...
class BasicLayer_LocalScan(nn.Module):
    def __init__(self,
                 dim,
                 input_resolution,
                 depth,
        ...
        self.blocks = nn.ModuleList()
        for i in range(depth):
            self.blocks.append(VSSBlock_LocalScan( # 如果SS2D每层传入的方向不同需要再这里传入
                hidden_dim=dim,
                drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                norm_layer=nn.LayerNorm,
                mlp_ratio=mlp_ratio,
...
# RSSG
class ResidualGroup_localScan(nn.Module):
    def __init__(self,
                 dim,
                 input_resolution,
                 depth,
                 ...
                 patch_size=None,
                 resi_connection='1conv'):
        super(ResidualGroup_localScan, self).__init__()

        self.dim = dim
        self.input_resolution = input_resolution # [64, 64]

        self.residual_group = BasicLayer_LocalScan(
            dim=dim,
            input_resolution=input_resolution,
            depth=depth,
            mlp_ratio=mlp_ratio,
...
class MambaIR_LocalScan(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 in_chans=3,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(MambaIR_LocalScan, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        self.img_range = img_range
...
class RSSG_Blocks_LocalScan(nn.Module):
    def __init__(self, num_blocks):
        super(RSSG_Blocks, self).__init__()
        self.layers = nn.ModuleList()
        self.num_blocks = num_blocks
...
if __name__ == '__main__':
    ...
    patch_embed = PatchEmbed(
            img_size=64, patch_size=16, in_chans=0, embed_dim=64, norm_layer=None)
    # N,L,C
    x = patch_embed(x)
    # layer = HybridAttBlock(dim=64).cuda() # # Vim + CA
    layer = ResidualGroup_localScan(dim=64, input_resolution=(64,64), depth=5,img_size=64,patch_size=16).cuda()


    output = layer(x,x_size)
    print(output.shape)
    # net = RSSG_Blocks(5).cuda()
    # output = net(x)
    # print(output.shape)