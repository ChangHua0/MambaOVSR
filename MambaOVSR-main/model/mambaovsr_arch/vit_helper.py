...
class PatchEmbed3D(nn.Module):
    ...
    """
    def __init__(
            self, img_size, temporal_resolution=4, in_chans=3, 
...
class PatchUnembed3D(nn.Module):
    """ Patch Embedding to Image (Inverse of PatchEmbed3D) """
    def __init__(
            self, img_size, temporal_resolution=4, embed_dim=768, 
            patch_size=16, z_block_size=2, in_chans=3, flatten=True
        ):
        super().__init__()
        self.height = img_size[0] // patch_size
        self.width = img_size[1] // patch_size
        self.frames = temporal_resolution // z_block_size
        self.num_patches = self.height * self.width * self.frames
        self.unproj = nn.ConvTranspose3d(embed_dim, in_chans,
            kernel_size=(z_block_size, patch_size, patch_size), 
            stride=(z_block_size, patch_size, patch_size))
        self.flatten = flatten
    ...
    def forward(self, x):
        if self.flatten:
            B, _, _ = x.shape
            x = x.transpose(1, 2).view(B, -1, self.frames, self.height, self.width)
        
        # Apply the transposed convolution to reverse the patch embedding
        x = self.unproj(x)
        return x
