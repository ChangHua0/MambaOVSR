...
class MultiScan(nn.Module):
    ...
    def multi_scan(self, x):
        ...
        """
        xs = []
        # 根据选择的扫描方向调用 scan 方法对输入进行扫描，返回一个列表 xs，其中每个元素是一个扫描方向的特征图
    ...
    def scan(self, x, direction='h'):
        ...
        if len(x.shape) == 3:
            if direction == 'h':
                ...
                K = int(direction[1:].split('_')[0]) # 窗口大小
                flip = direction.endswith('flip') # 是否翻转
                return local_scan(x, K, H, W, flip=flip) # 局部扫描
