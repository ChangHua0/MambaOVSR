...
class ImplicitWarpModule(nn.Module):
    ...
    def forward(self, y, x, flow):
        ...
        # 根据光流 flow 计算扭曲后的网格 grid_wrp
        grid_wrp = grid + flow.flip(dims=(-1,)) # grid_wrp
        # 计算网格的整数部分 grid_wrp_flr 和小数部分 grid_wrp_off
    ...
    # 用于计算单个点的正弦位置编码，使用正弦和余弦函数生成位置编码
    ...
    # 用于计算二维网格的正弦位置编码，生成一个形状为 [B, C, H, W] 的位置编码，并调整其形状以便后续处理
